...
async def clear_question_generation_cache():
    ...
    try:
        ...
        logger.error(f"Error clearing question generation cache: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to clear question generation cache: {str(e)}")
...
async def clear_answer_analysis_cache():
    ...
    try:
        ...
        logger.error(f"Error clearing answer analysis cache: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to clear answer analysis cache: {str(e)}")
...
async def reset_cache_metrics():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to reset cache metrics: {str(e)}")
